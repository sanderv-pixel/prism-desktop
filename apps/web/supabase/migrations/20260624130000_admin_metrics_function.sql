...
AS $$
  SELECT json_build_object(
    'visits', json_build_object(
      ...
      'week', (SELECT count(*) FROM public.visits WHERE created_at >= date_trunc('week', now())),
      'month', (SELECT count(*) FROM public.visits WHERE created_at >= date_trunc('month', now())),
      'uniqueToday', COALESCE((SELECT count(DISTINCT ip) FROM public.visits WHERE created_at >= date_trunc('day', now()) AND ip IS NOT NULL), 0)
    ),
